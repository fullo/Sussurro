...
pub fn agglomerative(embs: &[&[f32]], threshold: f32) -> Vec<usize> {
    ...
    loop {
        ...
        for k in (0..n).filter(|&k| alive[k] && k != a && k != b) {
            let merged =
                (size[a] as f32 * sim[a][k] + size[b] as f32 * sim[b][k]) / (size[a] + size[b]) as f32;
            sim[a][k] = merged;
            sim[k][a] = merged;
        }
        size[a] += size[b];
...
pub fn fold_small(embs: &[&[f32]], durations_ms: &[u64], labels: &[usize], min_ms: u64) -> Vec<usize> {
...
pub(crate) mod tests {
    ...
    fn online_separates_three_voices_with_stable_indices() {
        ...
        let got: Vec<usize> = order.iter().map(|&s| c.assign(&sample(&mut rng, &v[s], 1.0))).collect();
    ...
    fn agglomerative_finds_the_voices_and_is_deterministic() {
        ...
        let embs: Vec<Vec<f32>> = truth.iter().map(|&s| sample(&mut rng, &v[s], 0.9)).collect();
        ...
        assert_eq!(got, agglomerative(&refs, crate::speakers::REDETECT_THRESHOLD));
    ...
    fn small_clusters_fold_into_the_nearest_voice() {
        ...
        let spec = [(0, 0, 6000), (1, 1, 6000), (0, 0, 6000), (2, 1, 4000), (1, 1, 6000), (3, 0, 3000)];
        let embs: Vec<Vec<f32>> = spec.iter().map(|&(_, s, _)| sample(&mut rng, &v[s], 0.9)).collect();
        let refs: Vec<&[f32]> = embs.iter().map(Vec::as_slice).collect();
        let labels: Vec<usize> = spec.iter().map(|s| s.0).collect();
        let durs: Vec<u64> = spec.iter().map(|s| s.2).collect();
        assert_eq!(fold_small(&refs, &durs, &labels, 10_000), vec![0, 1, 0, 1, 1, 0]);
    ...
    fn renumbering_follows_first_appearance() {
        assert_eq!(renumber_by_first_appearance(&[5, 5, 2, 9, 2, 5]), vec![0, 0, 1, 2, 1, 0]);
    }
}
