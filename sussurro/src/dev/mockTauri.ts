...
let nextSession = 1;
let mic: { id: number; itemId: string; timer: number; n: number; started: number; saveAudio: boolean } | null = null;
const FAKE_LINES = [
...
function stopMic(): number {
  ...
  setTimeout(() => {
    ...
    if (m.saveAudio) s.audio = mockAudio((Date.now() - m.started) / 1000);
