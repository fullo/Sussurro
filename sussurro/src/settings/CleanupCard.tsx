...
import { bundledNote, cleanupProfile, isBundled, profileSummary } from "../lib/llmProfiles";
...
export function CleanupCard({
  ...
  /** Profiles are edited in Recipes: this opens it. */
  onEditProfiles: () => void;
}) {
  const { settings, setSettings, save, modelAdoptNote, defaultPrompts } = ctl;
  const profile = cleanupProfile(settings);
  return (
    <Card title={<>Cleanup <span className="via">via {profile?.name ?? "LLM"}</span></>}>
      <div className="field">
        <div className="field-label">
      ...
      {isBundled(profile) && <p className="card-hint">{bundledNote(ctl.bundledLlm)}</p>}
