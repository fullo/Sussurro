...
export function panelView(s: PanelState | null): PanelView {
  ...
  if (!s.paired) return view(problemText("not_paired"), "problem");
  ...
  switch (s.phase) {
    ...
    case "reconnecting": {
      const why = s.problem === "not_running" || s.problem === "timeout" ? " The app is not reachable." : "";
    ...
    case "idle":
    default:
      // Start stays available: it checks the app again first.
      if (s.app && !s.app.ok) return view(problemText(s.app.problem), "problem", true);
