...
import { SourceTracker, csrcId, toPerfTime } from "./sources";
import { assess, sameReport } from "./health";
import { cleanName, nameKey, tileKey } from "./guard";
...
describe("name guard", () => {
  it("keeps people's names as shown, whitespace collapsed", () => {
    ...
    expect(cleanName("Anna (Organisation)")).toBe("Anna (Organisation)");
  });
  ...
  it("strips the platforms' qualifiers (Teams, Zoom), localized, and keeps other parentheses", () => {
    expect(cleanName("Anna Rossi (Guest)")).toBe("Anna Rossi");
    expect(cleanName("Anna Rossi  (Ospite)")).toBe("Anna Rossi");
    expect(cleanName("Bruno (Unverified) (External)")).toBe("Bruno");
    expect(cleanName("Chiara (Host, me)")).toBe("Chiara");
    expect(cleanName("Dario (Co-host)")).toBe("Dario");
    expect(cleanName("Elena (Esterno)")).toBe("Elena");
    expect(cleanName("Franco (Contoso)")).toBe("Franco (Contoso)");
    expect(cleanName("Gina (Guest, Contoso)")).toBe("Gina (Guest, Contoso)");
    // Namesakes once the qualifier is gone: the binder refuses them.
    expect(nameKey(cleanName("Anna (Guest)")!)).toBe(nameKey(cleanName("anna")!));
  });

  it("rejects unknown callers, cut names and bare qualifiers", () => {
    for (const bad of ["Unknown user", "Utente sconosciuto", "Unbekannter Benutzer", "Anna Ro…", "Anna Ro...", "(Guest)", "Guest", "(Host, me)", "Unknown user (External)"]) {
...
describe("CSRC tracker", () => {
  it("turns fresh contributing sources into active/idle with the packet times", () => {
    const c = new SourceTracker({ hangMs: 400 });
  ...
  it("ignores quiet packets and a mirror on two receivers", () => {
    const c = new SourceTracker({ hangMs: 400, mirrorPolls: 3, minLevel: 0.01 });
